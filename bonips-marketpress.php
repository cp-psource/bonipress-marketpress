...
$myUpdateChecker = PucFactory::buildUpdateChecker(
	'https://github.com/cp-psource/bonips-marketpress',
	__FILE__,
	'bonips-marketpress'
);
...
if ( ! class_exists( 'boniPS_MarketPress_Plugin' ) ) :
	final class boniPS_MarketPress_Plugin {
		...
		public function __construct() {
			...
			$this->slug        = 'bonips-marketpress';
			$this->plugin      = plugin_basename( __FILE__ );
			$this->domain      = 'bonips_market';
			$this->plugin_name = 'boniPS for MarketPress';

			$this->define_constants();
			$this->includes();
			$this->plugin_updates();

			register_activation_hook( BONIPS_MARKETPRESS, 'bonips_marketpress_activate_plugin' );

			add_action( 'bonips_init',                                array( $this, 'load_textdomain' ) );
			add_action( 'bonips_all_references',                      array( $this, 'add_badge_support' ) );

			// Payment gateway
			add_action( 'mp_load_gateway_plugins',                    'bonips_marketpress_load_gateway' );
			add_action( 'marketpress/load_plugins/mp_include',        'bonips_marketpress_load_gateway' );
			...
			add_filter( 'bonips_parse_log_entry_marketpress_payment', 'bonips_marketpress_parse_log', 90, 2 );
			add_filter( 'bonips_email_before_send',                   'bonips_marketpress_parse_email', 20 );

			// Rewards
			add_action( 'bonips_load_hooks',                          'bonips_marketpress_load_rewards', 79 );
		...
		public function define_constants() {
			...
			$this->define( 'BONIPS_MARKETPRESS',        __FILE__ );
			$this->define( 'BONIPS_MARKET_ROOT_DIR',    plugin_dir_path( BONIPS_MARKETPRESS ) );
		...
		public function includes() {
			...
			$this->file( BONIPS_MARKET_INC_DIR . 'marketpress-gateway.php' );
			$this->file( BONIPS_MARKET_INC_DIR . 'marketpress-rewards.php' );
		...
		public function add_badge_support( $references ) {
			...
			$references['marketpress_payment'] = __( 'Shop Zahlung (MarketPress)', 'bonips_market' );
			$references['marketpress_sale']    = __( 'Shop Angebot (MarketPress)', 'bonips_market' );
			$references['marketpress_reward']  = __( 'Shop Belohnung (MarketPress)', 'bonips_market' );
...
function bonips_for_marketpress_plugin() {
	return boniPS_MarketPress_Plugin::instance();
}
bonips_for_marketpress_plugin();
 ...
 * @version 1.0
 */
function bonips_marketpress_activate_plugin() {
	...
	if ( defined( 'boniPS_VERSION' ) && version_compare( boniPS_VERSION, '1.7', '<' ) )
		$message[] = __( 'Diese Erweiterung erfordert BoniPress 1.7 oder höher. Ältere Versionen von BoniPress bieten integrierte Unterstützung für MarketPress, wodurch dieses Plugin überflüssig wird.', 'bonips_market' );
