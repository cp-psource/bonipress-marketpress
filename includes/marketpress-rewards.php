 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_marketpress_load_rewards' ) ) :
	function bonips_marketpress_load_rewards() {
...
if ( ! function_exists( 'bonips_markpress_payout_rewards' ) ) :
	function bonips_markpress_payout_rewards( $order ) {
		...
		if ( ! isset( $payment_info['gateway_private_name'] ) || ( $payment_info['gateway_private_name'] == BONIPS_DEFAULT_LABEL && apply_filters( 'bonips_marketpress_reward_bonips_payment', false ) === false ) )
			return;
		...
		foreach ( $types as $type => $label ) {
			...
			if ( $reward != $bonips->zero() ) {
				...
				$bonips->add_creds(
					'marketpress_reward',
					$order->user_id,
					apply_filters( 'bonips_marketpress_reward_log', '%plural% Belohnung für den Kauf im Shop', $order_id, $type ),
					$log,
					$order->ID,
					array( 'ref_type' => 'post' ),
					$type
